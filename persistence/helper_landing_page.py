...
from persistence.BaseModel import PageEntity, LandingPageEntity, RedirectionPathAssociation, LandsAssociation, DomainNameEntity
...
def insert(domain_name: str, landing_url: str, redirection_path: List[str], hsts: bool):
    ...
    for page in redirection_path:
        ...
        RedirectionPathAssociation.get_or_create(landing_page=lp.page, page=p)
...
def get_from_domain_name(domain_name: str) -> Tuple[str, List[str], bool]:      # da testare e mettere a posto
    ...
    for row in query:
        ...
        q = RedirectionPathAssociation.select()\
            .join_from(RedirectionPathAssociation, LandingPageEntity)\
            .where(RedirectionPathAssociation.landing_page_id == landing_page.id)
        for r in q:
            print(r.page_id)
