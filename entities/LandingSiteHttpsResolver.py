...
class LandingSiteHttpsResolver:
    """
    This class concern is, given an url/domain name, to find the landing url while keeping tracks of all redirection
    happened. Start the request using https. An instance of this class already makes the elaboration in the initialization, no need to invoke
    ...
    def __init__(self, domain_name: str):
        ...
        try:
            response = requests.get(domain_name_utils.deduct_http_url(domain_name, as_https=True))
            bool_list = list()
            ...
            raise
        except requests.exceptions.ConnectionError:  # A Connection error occurred.
            raise
