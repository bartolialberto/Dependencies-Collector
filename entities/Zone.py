...
class Zone:
    ...
    def __init__(self, zone_name: str, list_rr_a_of_nsz: list, list_cnames: list):
        if not list_utils.is_list_of_type_RRecord_and_rr_type(list_rr_a_of_nsz, TypesRR.A):
            raise ValueError()
        if list_rr_a_of_nsz is None or len(list_rr_a_of_nsz) == 0:
            self.zone_nameservers = list()
        else:
            self.zone_nameservers = list_rr_a_of_nsz
        if not list_utils.is_list_of_type_RRecord_and_rr_type(list_cnames, TypesRR.CNAME):
            raise ValueError()
