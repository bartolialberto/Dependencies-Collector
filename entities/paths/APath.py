...
class APath(Path):
    def __init__(self, rr_list: List[RRecord]):
        ...
        if rr_list[-1].type != TypesRR.A:
            raise ValueError("Path doesn't resolve in an A type RR.")
    ...
    def get_path_from(self, domain_name: DomainName) -> 'APath':        # FORWARD DECLARATIONS (REFERENCES)
        for i, rr in enumerate(self.path):
            if rr.name == domain_name:
                return APath(self.path[i:])
        raise DomainNameNotInPathError(domain_name)
