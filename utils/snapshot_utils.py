...
def take_temporary_snapshot(web_sites: List[str], mail_domains: List[str], persist_errors: bool, consider_tld: bool) -> None:
...
def take_temp_snapshot_of_flags(persist_errors: bool, consider_tld: bool, filename: str, project_root_directory=Path.cwd()) -> None:
    ...
    file = file_utils.set_file_in_folder("SNAPSHOTS", filename + ".txt",
                                         project_root_directory=project_root_directory)
    with file.open('w', encoding='utf-8') as f:  # 'w' or 'x'
        f.write('persist_errors:'+str(persist_errors)+'\n')
